...
function print(x, reportError = false, dumphex = false) {
    ...
    xhr.open("GET", localHost + "api/first?" + req , false);
    xhr.send(null);
}
function redirect()
{
...
async function getJS(fname, method = 'GET') {
  try {
    const url = fname;
    const xhr = new XMLHttpRequest();
    
    // 改用异步 true
    xhr.open(method, localHost + url, true);
    
    return new Promise((resolve, reject) => {
      xhr.onload = function() {
        if (xhr.status >= 200 && xhr.status < 300) {
          resolve(xhr.responseText); // 成功返回 JS 字符串
        } else {
          reject(new Error(`请求失败：${xhr.status}`));
        }
      };
      xhr.onerror = () => reject(new Error("网络错误或跨域"));
      xhr.send();
    });
  } catch (e) {
    throw e;
